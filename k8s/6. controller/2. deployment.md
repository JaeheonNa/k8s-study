...
##### Deployment는 replicaSet을 control하는 방식을 통해 위 배포 전략들 중 recreate와 rolling update를 지원하기 위한 object임. 
...
##### deployment rollback은 아래와 같이 할 수 있음.

    kubectl rollout undo deployment <Deployment명> --to-revision=<돌아갈Revision>
    ex) kubectl rollout undo deployment <deployment-1> --to-revision=2

##### revision 이력은 아래와 같은 명령어로 확인할 수 있음.

    kubectl rollout history deployment <Deployment명>
    ex) kubectl rollout history deployment <deployment-1> 